<?php
/**
 * NaPlanta - Gerador de Feed XML (formato VivaReal/OLX) via API Imoview em tempo real
 *
 * Substitui o feed estático hospedado em S3
 * (https://universal-ftp2.s3.us-west-2.amazonaws.com/vr/naplanta-vivareal-6873-ambos.xml)
 ...
 *
 * Para usar: aponte o portal (OLX/Grupo OLX/VivaReal) para a URL pública deste
 * arquivo, ex: https://www.virtualnaplanta.com.br/instagram/feed_olx.php
 */
...
$CACHE_FILE = __DIR__ . '/cache_feed_olx.xml';
...
$OLX_CONFIG_FILE = __DIR__ . '/olx_config.json';
$olx_config      = file_exists($OLX_CONFIG_FILE) ? json_decode(file_get_contents($OLX_CONFIG_FILE), true) : null;
$olx_config      = $olx_config ?: ['limit' => 150, 'qualif' => [], 'endereco' => 'completo', 'endereco_aluguel' => 'completo'];
$qualif_map      = is_array($olx_config['qualif'] ?? null) ? $olx_config['qualif'] : [];
$limit_total     = (int)($olx_config['limit'] ?? 150);
$endereco_modo_venda   = (($olx_config['endereco'] ?? 'completo') === 'aproximado') ? 'aproximado' : 'completo';
$endereco_modo_aluguel = (($olx_config['endereco_aluguel'] ?? 'completo') === 'aproximado') ? 'aproximado' : 'completo';
$renovacao_ativa       = !isset($olx_config['renovacao']) || $olx_config['renovacao'] !== false;
...
// Ordem de prioridade da qualificacao (maior primeiro)
$ordem_qualif = ['exclusivo' => 4, 'super' => 3, 'destaque' => 2, 'sem' => 1];

// Mapa qualificacao -> PublicationType (VRSync). Confirmar com o suporte da
// OLX/integrador se "PREMIUM_1"/"PREMIUM_2" sao os identificadores corretos
// para Super Destaque / Destaque Exclusivo na conta da imobiliaria.
$publication_map = [
    'sem'       => 'STANDARD',
    'destaque'  => 'PREMIUM',
    'super'     => 'PREMIUM_1',
...
foreach ($todos as $im) {
    ...
    // Campos com faixa de validação no Grupo OLX (ex.: área útil 10–5000, quartos/banheiros 1–40)
